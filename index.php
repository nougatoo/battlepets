...
/* Getting URLs for each realms 
// First we have to move all data from the hourly to the daily table
...
*/
/* End of getting auctions data and inserting */

//print_r($dataUrls);

/* End of multiple curl */

/* Getting All Auction House Pet Data

$realmsToPull = [];
$realmsCompleted = []; // Used to know which realms + connected realms we've done
$sql = "SELECT slug FROM realms";
$result = $conn->query($sql);
...
foreach($realmsToPull as $realmPulling) {
	
	// So that we don't pull connected realm data twice
	if(!in_array($realmPulling, $realmsCompleted))
	{
		
		$startTimeTotal = microtime(true);
		// Inital call to get URL for Realm
		$content = file_get_contents("https://us.api.battle.net/wow/auction/data/".$realmPulling."?locale=en_US&apikey=r52egwgeefzmy4jmdwr2u7cb9pdmseud");
		$result  = json_decode($content, true);
		$url = $result['files'][0]['url'];
			
		// Getting the actual auction data for realm
		...
		$timeDiffTotal = $endTimeTotal - $startTimeTotal;
		$startTimeTotal = microtime(true);
		file_put_contents('php://stderr', print_r($realmPulling . ": " . $timeDiffTotal . " - " , TRUE));

		// Map each name to a slug so we don't have to query db
		foreach($ahRealms as $aRealm) {
			
			array_push($realmSlugList, $aRealm['slug']);
			array_push($realmNameList, $aRealm['name']);
			array_push($realmsCompleted, $aRealm['slug']); // Warning...adding to completed list before pulling is done
			
			echo("Working on: " . $aRealm['slug'] . "<br/>");
			
		}

		$slugMap = array_combine($realmNameList, $realmSlugList);

		foreach ($auctions as $currentAuction) {
			...
			if($isPet)
			{
				...
				$sql = "INSERT INTO auctions_daily_pet (id, species_id, realm, buyout, bid, owner, time_left, quantity)
				VALUES ('" . $id . "', '" . $speciesId . "', '" . $slugMap[$realmName] . "', '" . $buyout . "', '" . $bid . "', '" . $owner . "', '" . $timeLeft . "', '" . $quantity . "')"
						. "ON DUPLICATE KEY UPDATE "
						. "bid='" . $bid  . "', time_left='" . $timeLeft  . "'";
		...
		$endTimeTotal = microtime(true);
		$timeDiffTotal = $endTimeTotal - $startTimeTotal;
		file_put_contents('php://stderr', print_r("Processing Auctions time: " . $timeDiffTotal . "\n" , TRUE));
